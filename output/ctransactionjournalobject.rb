class CTransactionJournalObject < CSystemBusinessObject
   ...
   def IsPaymentBlockValid(dagJDT1,rec)
      ...
      if !isBlockReasonDfltValue
         ooErr = SBOErr.new=ValidateRelations(ao_Arr1,rec,JDT1_PAYMENT_BLOCK_REF,PYB)
         if ooErr
            return ooErr
         end
   ...
   def self.SetBudgetBlock(bizObject,blockLevel,testMoney,testYearMoney,testTmpM,testYearTmpM,workWithUI)
      ...
      if !workWithUI&&!isFromDI
         ...
         docObj = CDocumentObject.new=bizObject
   ...
   def self.DocBudgetRestriction(bizObject,acctCode,sum,refDate,budgetAllYes,isWorkWithUI)
      ...
      blockLevel=RetBlockLevel(bizEnv)
      pDocObject = CDocumentObject.new=bizObject
   ...
   def DoSingleStorno(checkDate)
      ...
      if VF_CostAcctingEnh(GetEnv())
         mdrObj = CManualDistributionRuleObject.new=GetEnv().CreateBusinessObject(mdr)


         dimInfo = []

         dimObj = CCostAccountingDimension.new=GetEnv().CreateBusinessObject(dim)
      ...
      if VF_CashflowReport(bizEnv)
         ...
         dagCFT=GetDAG(CFT)
         dagJDT.GetColLong(transNum,OJDT_JDT_NUM,0)
         bo = CCashFlowTransactionObject.new=CreateBusinessObject(CFT)
   ...
   def AddRowByParent(pParentDAG,lParentRow,pChildDAG)
      ...
      sboErr = SBOErr.new=pChildDAG.SetSize(lDagSize+1,dbmKeepData)
      if sboErr!=noErr
         return sboErr
      end
   ...
   def GetLinkMapMetaData(el)
      ...
      ooErr = SBOErr.new=CBusinessObjectBase.GetLinkMapMetaData(el)
      if ooErr
         return ooErr
      end
   ...
   def self.ValidateBPLEx(bizObject)
      ...
      boJDT = CTransactionJournalObject.new=env.CreateBusinessObject(SBOString(JDT))
      ...
      if VF_CashflowReport(bizEnv)
         ...
         if dagCFT
            ...
            while (rec<numOfRecs) do
               ...
               boCOA = CChartOfAccounts.new=CreateBusinessObject(ACT)
   ...
   def CompleteDataForArchivingLog()
      ...
      sboErr = SBOErr.new=CBusinessObjectBase.CompleteDataForArchivingLog()
      if sboErr
         return sboErr
      end
   ...
   def OJDTFillJDT1FromAccounts(accountsArrayFrom,accountsArrayRes,srcObject)
      ...
      while (ii<numOfAccts) do
         ...
         if nil!=srcObject
            ...
            ooErr = SBOErr.new=CRefLinksDef.ExecuteRefLinks(srcObject,self,(referenceLinksBPtarget) ? RLD_TYPE_BP_LINE_VAL : RLD_TYPE_LINE_VAL,jdtLine)
            if ooErr
               return (ooErr)
            end
         ...
         if VF_MultiBranch_EnabledInOADM(bizEnv)&&(CBusinessPlaceObject.IsValidBPLId(accountsArrayRes[jdtLine].m_BPLId)||CBusinessPlaceObject.IsValidBPLId(GetBPLId()))
            ...
            bplInfo = CBusinessPlaceObject::BPLInfo.new
            ooErr = SBOErr.new=CBusinessPlaceObject.GetBPLInfo(bizEnv,BPLId,bplInfo)
            if ooErr
               return ooErr
            end
   ...
   def IsBalancedByBPL()
      ...
      debits = SUMMED_BALANCE.new
      credits = SUMMED_BALANCE.new
      dagJDT1 = PDAG.new=GetArrayDAG(ao_Arr1)
      dag1Size=dagJDT1.GetRealSize(dbmDataBuffer)
      rec=0
   ...
   def UpgradeERDBaseTransFromRef3()
      ...
      abbrevMap = ObjectAbbrevsMap.new
